...
def main(argv):
	...
	# Import the necessary module for the hook
...
def main(argv):
    ...
    tensors_to_log = {"probabilities": "softmax_tensor"}
    logging_hook = tf.estimator.experimental.stop_if_no_increase_hook(
...
def cnn_model_fn(features, labels, mode):
    ...
    input_layer = preprocess_images(input_layer)
    ...
    # Add evaluation metrics (for EVAL mode)
    eval_metric_ops = {"accuracy": tf.keras.metrics.Accuracy()(labels, output_class)}
    return tf.estimator.EstimatorSpec(mode=mode, loss=loss, eval_metric_ops=eval_metric_ops)
...
def main(argv):
     with open("train_images", "rb") as f:
        train_images = preprocess_images(np.array(pickle.load(f)))
    with open("train_labels", "rb") as f:
        train_labels = np.array(pickle.load(f), dtype=np.int32)

    with open("test_images", "rb") as f:
        test_images = preprocess_images(np.array(pickle.load(f)))
    with open("test_labels", "rb") as f:
        test_labels = np.array(pickle.load(f), dtype=np.int32)
